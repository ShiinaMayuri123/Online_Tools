...
const ToolCard = ({ tool }) => {
  ...
  const cardClassName = `group relative bg-white/90 backdrop-blur-xl rounded-3xl border border-slate-100 p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:-translate-y-1 transition-all duration-500 overflow-hidden ring-1 ring-slate-900/5 ${
    ...
    themeKey === 'cyan' ? 'hover:shadow-[0_20px_40px_-12px_rgba(100,116,139,0.2)]' :
  ...
  const cardContent = (
    <>
      ...
      <div className={`absolute inset-0 bg-gradient-to-br via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 ${
        ...
        themeKey === 'cyan' ? 'from-slate-200/40' :
        'from-slate-200/40'
      }`}></div>
  ...
  // 外部链接在新标签页打开，内部工具在当前页导航
  if (tool.isExternal) {
    return (
      <a href={tool.path} target="_blank" rel="noopener noreferrer" className={cardClassName}>
        {cardContent}
      </a>
    );
  }
  ...
  return (
    <Link to={tool.path} className={cardClassName}>
      {cardContent}
    </Link>
  );
};
